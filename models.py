from flask import (Flask, request, render_template,
                   jsonify, send_file, redirect, url_for, session, flash)
...
def generate_pdf_report(analysis_data, candidates_data):
    ...
    c.setFont("Helvetica", 11)
    c.drawString(44, h-122, f"Total: {analysis_data['total']}  |  "
    ...
    for cand in candidates_data:
        ...
        c.drawString(44, y-14, f"Score: {cand['score']}/100  |  TF-IDF: {cand['tfidf_score']}%  |  ML: {cand['ml_prob']}%")
        if cand['matched_skills']:
            c.setFont("Helvetica", 9)
            c.setFillColor(colors.HexColor('#718096'))
            c.drawString(44, y-30, f"Skills: {', '.join(cand['matched_skills'])}")
        y -= 68
    c.save()
    buffer.seek(0)
    return buffer
    ...
    try:
        ...
        <div style="font-family:Segoe UI,sans-serif;max-width:600px;margin:0 auto;">
          <div style="background:#1a365d;padding:24px;border-radius:12px 12px 0 0;">
            <h1 style="color:white;margin:0;font-size:20px;">🤖 AI Candidate Selection Result</h1>
            <p style="color:rgba(255,255,255,0.8);margin:6px 0 0;">Sri Krishna Engineering</p>
          </div>
          <div style="background:#f7fafc;padding:24px;border-radius:0 0 12px 12px;">
            <p style="font-size:15px;color:#2d3748;">Analysis complete for <strong>{job_role}</strong></p>
            <div style="background:white;border-radius:10px;padding:20px;margin:16px 0;">
              ...
              <p style="margin:0;font-size:22px;font-weight:800;color:#1a365d;">🏆 {top_candidate}</p>
            </div>
            <div style="display:flex;gap:12px;">
              <div style="flex:1;background:white;border-radius:8px;padding:14px;text-align:center;">
                <p style="margin:0;font-size:22px;font-weight:800;color:#3182ce;">{total}</p>
                <p style="margin:4px 0 0;font-size:12px;color:#718096;">Resumes Analyzed</p>
              </div>
              <div style="flex:1;background:white;border-radius:8px;padding:14px;text-align:center;">
                <p style="margin:0;font-size:22px;font-weight:800;color:#38a169;">{avg_score}</p>
                <p style="margin:4px 0 0;font-size:12px;color:#718096;">Average Score</p>
              </div>
            </div>
...
def analyze():
    ...
    analysis_row = Analysis(
        job_role=job_role, total_resumes=len(candidates),
        top_candidate=candidates[0]["name"] if candidates else "",
        top_score=candidates[0]["score"]    if candidates else 0,
        avg_score=result["avg_score"], recommended=result["recommended"]
    )
    db.session.add(analysis_row)
    db.session.flush()
    for c in candidates:
        db.session.add(Candidate(
            analysis_id=analysis_row.id, name=c["name"],
            score=c["score"], tfidf_score=c["tfidf_score"],
            ml_prob=c["ml_prob"], ml_suitable=c["ml_suitable"],
            skill_pct=c["skill_pct"], has_exp=c["has_exp"],
            has_edu=c["has_edu"], matched_skills=", ".join(c["matched_skills"]),
